...
class SUB:
    ...
    def __init__(
        ...
        self.B = 0.75                 # beam (m)
        self.T = 0.4                  # draft (m)
        self.wn = 0.3                 # natural frequency for control
        self.zeta = 1                 # damping ratio
        self.tau_X = 10000            # surge force (N), pilot input
    ...
    def dynamics(self, eta, nu, u_actual, u_control, sampleTime):
        ...
        M = np.diag([[self.mass, self.mass, self.I_y], [self.mass, self.mass, self.I_y], [self.mass, self.mass, self.I_y]])
        ...
        '''
        # Hydrodynamic drag in surge and pitch
        C_x = -self.CD_0 * self.mass        # Surge drag coefficient
        M_q = -0.05 * self.I_y              # Pitch damping coefficient
        
        # Control forces vector [surge force, heave force (0), pitch moment]
        # Heave control is omitted here, and its force is set to 0
        tau = np.array([tau_surge - C_x * nu_r[0], 0, tau_pitch - M_q * nu_r[4]], float)
        
        # Inertia matrix for surge (DOF 1), heave (DOF 3), and pitch (DOF 5)
        # Only using relevant DOF (surge, heave, pitch), this is now a 3x3 matrix
        M = np.diag([self.mass, self.mass, self.I_y])  # 3x3 inertia matrix
        
        # Invert the inertia matrix
        Minv = np.linalg.inv(M)
        
        # Reduced velocities vector [nu_r]
        nu_reduced = np.array([nu_r[0], nu_r[2], nu_r[4]])  # Only surge, heave, pitch velocities
        
        # Calculate the rate of change in velocities [nu_dot]
        nu_dot_reduced = np.matmul(Minv, tau)
        
        # Forward Euler integration for velocity updates
        nu_reduced += sampleTime * nu_dot_reduced
        
        # Update the main velocity vector [nu] with the new surge, heave, and pitch values
        nu[0] = nu_reduced[0]  # Surge velocity
        nu[2] = nu_reduced[1]  # Heave velocity
        nu[4] = nu_reduced[2]  # Pitch velocity
        
        # Update control inputs (thrust, pitch)
        u_actual = u_control
    ...
    def depthAutopilot(self, eta, nu, sampleTime):
        ...
        '''
        [u_depth, self.z_int, self.ref_z, useless_1, useless_2] = PIDpolePlacement(
            self.z_int, 0, 0, 0, 0, 0, e_z, w, self.ref_z, 0, 0,
            self.mass, self.wn, self.zeta, 0, sampleTime
        )
    ...
    def pitchAutopilot(self, eta, nu, sampleTime):
        ...
        '''
        [u_pitch, self.theta_int, self.ref_theta, useless_1, useless_2] = PIDpolePlacement(
            self.theta_int, 0, 0, 0, 0, 0, e_theta, q, self.ref_theta, 0, 0,
            self.I_y, self.wn, self.zeta, 0, sampleTime
        )
